...
class SOTATrainer:
    ...
    def _save_fsdp2_checkpoint(self, path: str) -> None:
        ...
        # Save standard HF-style weights for checkpoint-best.
        # This keeps FSDP2 sharded distcp resume files and adds
        # model.safetensors-style artifacts expected by SOTA/DDP flows.
        if self._should_save_fsdp2_standard_weights(path):
            self._save_fsdp2_standard_weights(path)
    ...
    @staticmethod
    def _should_save_fsdp2_standard_weights(path: str) -> bool:
        """
        Emit HF-style model weights only for checkpoint-best.

        This avoids expensive full-parameter materialization for every
        periodic checkpoint while ensuring the best checkpoint is portable.
        """
        checkpoint_name = os.path.basename(os.path.normpath(path))
        return checkpoint_name == "checkpoint-best"

    def _save_fsdp2_standard_weights(self, path: str) -> None:
        """
        Save a standard HF-style model snapshot in an FSDP2 checkpoint dir.

        All ranks must enter summon_full_params(); only main process writes.
        """
        log_rank_0(
            "Materializing full params for standard checkpoint weights..."
        )
        ...
        with self._fsdp2_engine.summon_full_params(writeback=False):
            if self._is_main_process:
                if hasattr(model_to_save, "save_pretrained"):
                    try:
                        ...
                        log_rank_0(
                            f"Standard weights saved: {path}"
                        )
                    except Exception as e:
                        logger.warning(
                            "save_pretrained failed during FSDP2 "
                            f"checkpoint standardization: {e}. "
                            "Falling back to model.pt."
                        )
